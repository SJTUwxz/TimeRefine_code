...
class DataArguments:
    ...
    # segment format: v1  <SEG_START> time time <SEG_END>
    # segment format: v2  <SEG_START> <SEG_END> <SEG_FIN>
    # segment format: v3  <SEG_START> 00 to 99,
    # segment format: v4  <SEG_START> <00> <01> <02> ....
    # segment format: v5  <SEG_START> <00> <01> <02> ....
    segment_format: Optional[str] = field(default=None)
    predict_center_offset: Optional[bool] = field(default=False)
    add_temporal_tokens: Optional[str] = field(default="yes")
...
def preprocess_v1(
    ...
    # set tokens corrsponding to timestamps to -300
    # if data_args.segment_format == "v1":
    #     seg_start_id = tokenizer.convert_tokens_to_ids(SEG_START)
    #     seg_end_id = tokenizer.convert_tokens_to_ids(SEG_END)
    #     segment_start_indices = torch.where(input_ids[0,:] == seg_start_id)[0]+1
    #     segment_end_indices = torch.where(input_ids[0,:] == seg_end_id)[0]-1
    #     assert torch.all(segment_end_indices==segment_start_indices+1), "segment end != segment start + 1"
    #
    #     input_ids[0, segment_start_indices] = TEMPORAL_TOKEN_INDEX 
    #     input_ids[0, segment_end_indices] = TEMPORAL_TOKEN_INDEX 
    ...
    for conversation, target in zip(conversations, targets):
        ...
        if 0 in target:
            pass
            # indices = torch.nonzero(target == 0, as_tuple=True)[0].item()
        ...
        for i, rou in enumerate(rounds):
            ...
            target[cur_len : cur_len + instruction_len] = IGNORE_INDEX
            # if data_args.segment_format in ["v1", "v2"]:
            #     start_indices = torch.where(target == seg_start_id)[0]
            #     end_indices = torch.where(target == seg_end_id)[0]
            #     if data_args.segment_format == "v1":
            #         for i, index in enumerate(start_indices):
            #             target[index+1 : end_indices[i]] = IGNORE_INDEX 
            #     elif data_args.segment_format == "v2":
            #         for i, index in enumerate(start_indices):
            #             target[index+1 : end_indices[i]+1] = IGNORE_INDEX 
...
def preprocess(
    ...
    """
    assert conversation_lib.default_conversation.version.startswith("v1"), "only support conversation version == v1"
    if conversation_lib.default_conversation.sep_style == conversation_lib.SeparatorStyle.PLAIN:
        return preprocess_plain(sources, tokenizer)
    if conversation_lib.default_conversation.sep_style == conversation_lib.SeparatorStyle.LLAMA_2:
        return preprocess_llama_2(sources, tokenizer, has_image=has_image)
    if conversation_lib.default_conversation.version.startswith("v1"):
        return preprocess_v1(sources, tokenizer, has_image=has_image, data_args=data_args)
    # add end signal and concatenate together
    conversations = []
    for source in sources:
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'meta' in source and self.data_args.segment_format != 'v9':
            def convert(duration, x):
                ...
                if len(x) == 1:
                    x = "0" + x
                if self.data_args.add_temporal_tokens == "yes":
                    x = f"<{x}>"
                    return x
                # "yes_sep" means that <11> will be separated to <1><1>
                elif self.data_args.add_temporal_tokens == "yes_sep":
                    x = f"<{x[0]}><{x[1]}>"
                    return x
                else:
                    return x

            def convert_offset(duration, x):
                x = x / duration * 100
                if x >= 0:
                    symbol = "+"
                else:
                ...
                if len(x) == 1:
                    x = "0" + x
                if self.data_args.segment_format == "v10":
                    x = symbol + x
                    if self.data_args.add_temporal_tokens == "yes":
                        x = f"<{x}>"
                        return x
                        ...
                        return x
                elif self.data_args.segment_format in ["v11", "v12"]:
                    if self.data_args.add_temporal_tokens == "yes":
                        x = f"<{symbol}> <{x}>"
                        return x
                    elif self.data_args.add_temporal_tokens == "yes_sep":
            ...
            for k, v in source['meta']['token'].items():
                if "so" in k or "eo" in k:
                    replace_set.append((k, convert_offset(source['meta']['duration'], v)))
                else:
            ...
            segments = source['meta']['segment']
            segments = np.array(segments, dtype=np.float32)
            segments = segments / source['meta']['duration']
        ...
        count = torch.sum(data_dict["input_ids"] == 32003).item()
        if count == 0:
        ...
        assert (count * 2 == num_segments), f"number of segments should be twice the number of <OFFSET>. {conversations}, {input_ids}, {segments}"
        # print(source["conversations"], data_dict["input_ids"])
        # _to: 304, to: 517
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if "segment" in instances[0].keys():
            segments = [instance["segment"] for instance in instances]
