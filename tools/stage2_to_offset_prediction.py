...
# propose segment and offset and ensure that the iou is not below 0.3
def propose_segment_offset(start, end, std_dev):
    proposed_segments = []
    for std_ind, std in enumerate(std_dev):
        while True:
            new_start_tmp = np.random.normal(loc=start, scale=std)
            ...
            if new_end < new_start:
                ...
                new_start = new_end
                new_end = tmp
            if get_iou([new_start, new_end], [start, end]) < 0:
                continue
            else:
...
if __name__ == "__main__":
    ...
    # not randomizing the std_dev here
    ...
    for epoch in range(8):
        ...
        for data in tqdm(annotation):
            ...
            cur_token_length = len(new_tokens.keys())
            ...
            for i in range(length):
                ...
                if type_template == "temporal_grounding":
                    ...
                    std_dev = np.random.uniform(0, gt_duration*0.5, [num_segment]).tolist()
                    ...
                    for std_ind, item in enumerate(proposed_segments):
                        ...
                        if std_ind == 0:
                            answer = f"From <SEG_START> <s{cur_token_ind}> to <e{cur_token_ind}> <WITH_OFFSET> <so{cur_token_ind}> and <eo{cur_token_ind}>"
                        elif std_ind == len(std_dev) - 1:
                            answer = f"{answer} <RETHINK> <s{cur_token_ind}> to <e{cur_token_ind}> <WITH_OFFSET> <so{cur_token_ind}> and <eo{cur_token_ind}> <SEG_END>."
                        else:
                            answer = f"{answer} <RETHINK> <s{cur_token_ind}> to <e{cur_token_ind}> <WITH_OFFSET> <so{cur_token_ind}> and <eo{cur_token_ind}>"
                            ...
                            for std_ind, item in enumerate(proposed_segments):
                                ...
                                if std_ind == 0:
                                    if match_type == "v1":
                                        if event.startswith('. '):
                                            event = event[2:]
                                        answer = f"{event} from <SEG_START> <s{cur_token_length}> to <e{cur_token_length}> <WITH_OFFSET> <so{cur_token_length}> and <eo{cur_token_length}>"
                                    else:
                                        answer = f"From <SEG_START> <s{cur_token_length}> to <e{cur_token_length}> <WITH_OFFSET> <so{cur_token_length}> and <eo{cur_token_length}>"

                                elif std_ind == len(proposed_segments) - 1:
                                    if match_type == "v1":
                                        answer = f"{answer} <RETHINK> <s{cur_token_length}> to <e{cur_token_length}> <WITH_OFFSET> <so{cur_token_length}> and <eo{cur_token_length}> <SEG_END>."
                                    else:
                                        answer = f"{answer} <RETHINK> <s{cur_token_length}> to <e{cur_token_length}> <WITH_OFFSET> <so{cur_token_length}> and <eo{cur_token_length}> <SEG_END>, {event}."
                                else:
                                    answer = f"{answer} <RETHINK> <s{cur_token_length}> to <e{cur_token_length}> <WITH_OFFSET> <so{cur_token_length}> and <eo{cur_token_length}>"
    ...
    if grounding_only:
        output = f'./data/vtimellm_train/stage2_offset_{num_segment}_grounding.json'
        json.dump(output_annotation, open(output, 'w'), indent = 6)
    else:
        if transform_dense_captioning:
            output = f'./data/vtimellm_train/stage2_offset_{num_segment}_both.json'
            json.dump(output_annotation, open(output, 'w'), indent = 6)
            # json.dump(output_annotation, open('./data/vtimellm_train/stage2_offset.json', 'w'), indent = 6)
        else:
            output = f'./data/vtimellm_train/stage2_offset_{num_segment}.json'
            json.dump(output_annotation, open(output, 'w'), indent = 6)
    print(output)
