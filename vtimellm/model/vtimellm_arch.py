...
from vtimellm.constants import IMAGE_TOKEN_INDEX, IGNORE_INDEX, TEMPORAL_TOKEN_INDEX, SEG_START, SEG_END
...
class TimestepEmbedder(nn.Module):
    ...
    def __init__(self, hidden_size, frequency_embedding_size=256):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(frequency_embedding_size, hidden_size, bias=True).to(torch.float32),
            nn.SiLU(),
            nn.Linear(hidden_size, hidden_size, bias=True).to(torch.float32),
        )
...
class VTimeLLMMetaModel:
    ...
    def initialize_vision_modules(self, model_args):
        ...
        #         if self.model_args.projector_type == "dit":
        #             self.t_embedder = TimestepEmbedder(self.config.hidden_size)
        #
        #             nn.init.normal_(self.t_embedder.mlp[0].weight, std=0.02)
        #             nn.init.normal_(self.t_embedder.mlp[2].weight, std=0.02)
        #         else:
        #             raise NotImplementedError("temporal projector not implemented")
