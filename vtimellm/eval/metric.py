...
def offset_captioning_metrics_temporal_tokens(all_logs, data_path):
    ...
    for log in logs:
        ...
        try:
            ...
            for item in items:
                ...
                timestamps = list(filter(lambda x: x != '', timestamps))
                last_rethink = len(timestamps) - 1 - timestamps[::-1].index("<RETHINK>")
                start = int(timestamps[last_rethink+1][1:-1])
                end = int(timestamps[last_rethink+3][1:-1])
                pred[id].append({
                        'timestamp': [start, end],
                        'sentence': item['event'],
                    })
...
def offset_captioning_metrics(all_logs, data_path):
    logs = [x for x in all_logs if x['task'] == 'captioning']
    pred = {}
    for log in logs:
        id = log['video_id']
        ...
        try:
            items = json.loads(re.search(r'\[.*\]', answer).group(0))
            for item in items:
                timestamps = item['timestamps'].split("  ")[-2].split(' ')
                start = int(timestamps[0])
                end = int(timestamps[2])
                pred[id].append({
                        'timestamp': [start, end],
                        'sentence': item['event'],
                    })
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task", type=str, default='all', choices=['all', 'grounding', 'captioning', 'offset_captioning', 'offset_captioning_temporal_tokens'])
    ...
    if args.task in ['offset_captioning', 'all']:
        print("====================== Captioning =====================")
        print_metrics(offset_captioning_metrics(logs, args.data_path))
    if args.task in ['offset_captioning_temporal_tokens', 'all']:
        print("====================== Captioning =====================")
        print_metrics(offset_captioning_metrics_temporal_tokens(logs, args.data_path))
